...
const IssueList = ({
  ...
  return <div className="space-y-4">
      {issues.map(issue => <div key={`${issue.repository?.fullName}-${issue.number}`} className="relative py-4 group">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-col md:flex-row md:items-start justify-between">
                <div className="flex-1">
                  <a href={issue.url} target="_blank" rel="noopener noreferrer" className="text-sm md:text-base font-medium text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white transition-colors">
                    {issue.title}
                  </a>
                ...
                <div className="flex flex-wrap gap-1.5 mt-2 md:mt-0 md:ml-4">
                  {issue.labels.map(label => <span key={label.name} className="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full whitespace-nowrap max-w-[120px]" style={getLabelColors(label.color)} title={label.name}>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 md:gap-0 text-sm">
                ...
                <div className="flex flex-wrap items-center gap-2 md:space-x-3">
                  <button onClick={() => onViewComments(issue)} className="flex-1 md:flex-none inline-flex items-center justify-center px-3 py-1.5 text-xs md:text-sm text-gray-600 dark:text-gray-300 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900 rounded-md transition-colors">
      ...
      {hasMore && <div className="flex justify-center mt-6">
          <button onClick={onLoadMore} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors">
            Load More
          </button>
        </div>}
    </div>;
