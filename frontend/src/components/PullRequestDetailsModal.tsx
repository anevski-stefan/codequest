...
export default function PullRequestDetailsModal({
  isOpen,
  onClose,
  pullRequestDetails,
  isLoading
  ...
  const tabContent = useMemo(() => {
    ...
    switch (activeTab) {
      ...
      case 'changes':
        return pullRequestDetails?.files?.length ? <div className="space-y-4">
            {selectedCommitSha && <div className="mb-4 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
            ...
            const formatHunkHeader = (line: string) => {
              ...
              return <div className="flex items-center gap-2 py-1 text-gray-500 dark:text-gray-400 text-sm">
            ...
            return <div key={file.filename} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                  <button onClick={() => toggleFile(file.filename)} className="w-full flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 hover:bg-gray-100 dark:hover:bg-gray-700/70 transition-colors">
                  ...
                  {isExpanded && file.patch && <div className="p-4 overflow-x-auto border-t border-gray-200 dark:border-gray-700">
                      <pre className="text-sm font-mono whitespace-pre">
                        {file.patch.split('\n').map((line, index) => {
                    if (line.startsWith('@@')) {
                      return <div key={index} className="border-t border-b border-blue-100 dark:border-blue-900 bg-blue-50 dark:bg-blue-950/30 px-2 my-2">
                                {formatHunkHeader(line)}
                              </div>;
                    }
                    return <div key={index} className={`px-2 ${line.startsWith('+') ? 'bg-green-100 dark:bg-green-950/30 text-green-800 dark:text-green-300' : line.startsWith('-') ? 'bg-red-100 dark:bg-red-950/30 text-red-800 dark:text-red-300' : 'text-gray-800 dark:text-gray-200'}`}>
                              {line}
                            </div>;
                  })}
                      </pre>
            ...
            <Dialog.Panel className="relative bg-white dark:bg-gray-800 rounded-xl w-full max-w-6xl max-h-[90vh] overflow-hidden shadow-xl">
              ...
              <div className="border-b border-gray-200 dark:border-gray-700">
                <nav className="flex space-x-8 px-4" aria-label="Tabs">
                  <button onClick={() => setActiveTab('changes')} className={`flex items-center gap-2 py-4 text-sm font-medium border-b-2 ${activeTab === 'changes' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'}`}>
                    <FileText className="w-4 h-4" />
                    <span>{pullRequestDetails?.changed_files} changed files</span>
                  </button>
                  <button onClick={() => setActiveTab('commits')} className={`flex items-center gap-2 py-4 text-sm font-medium border-b-2 ${activeTab === 'commits' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 dark:text-gray-400 dark:hover:text-gray-300'}`}>
