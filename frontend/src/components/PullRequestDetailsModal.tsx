...
export default function PullRequestDetailsModal({
  ...
  const tabContent = useMemo(() => {
    ...
    switch (activeTab) {
            ...
            return <div key={file.filename} className="border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
                  ...
                  {isExpanded && file.patch && <div className="p-4 overflow-x-auto border-t border-gray-200 dark:border-gray-700">
                      <pre className="text-sm font-mono whitespace-pre">
                        {file.patch.split('\n').map((line, index) => <div key={index} className={`${line.startsWith('+') ? 'bg-green-100 dark:bg-green-950/30 text-green-800 dark:text-green-300' : line.startsWith('-') ? 'bg-red-100 dark:bg-red-950/30 text-red-800 dark:text-red-300' : line.startsWith('@') ? 'bg-blue-100 dark:bg-blue-950/30 text-blue-800 dark:text-blue-300' : 'text-gray-800 dark:text-gray-200'}`}>
                            {line}
                          </div>)}
                      </pre>
                    </div>}
      ...
      case 'commits':
        return <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
            <div className="flex items-center gap-2">
