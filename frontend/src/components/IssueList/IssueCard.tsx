...
export const IssueCard = ({
  issue,
  onViewComments
}: IssueCardProps) => <div key={`${issue.repository?.fullName}-${issue.number}`} className="relative py-4 group">
      ...
      <div className="flex flex-col gap-3">
        <div className="flex flex-col md:flex-row md:items-start justify-between">
          ...
          <div className="flex flex-wrap gap-1.5 mt-2 md:mt-0 md:ml-4">
            {issue.labels.map(label => <IssueLabel key={label.name} name={label.name} color={label.color} />)}
          </div>
        </div>
