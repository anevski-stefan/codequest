...
const ThinkingAnimation = () => <div className="font-mono text-sm text-blue-500 dark:text-blue-400 animate-pulse flex items-center h-8">
...
const CodeBuddy = () => {
  ...
  const chatMutation = useMutation({
    mutationFn: async ({
      ...
      try {
        ...
        if (error && typeof error === 'object' && ('name' in error && error.name === 'AbortError' || 'message' in error && typeof error.message === 'string' && (error.message.includes('canceled') || error.message.includes('cancelled')))) {
  ...
  const renderMessage = (msg: Message, idx: number) => {
    ...
    return <div key={idx} className={`flex items-start gap-2 w-full ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
        {msg.role === 'assistant' && <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0 mt-1">
            <Bot className="w-5 h-5 text-white" />
          </div>}
        
        <div className="max-w-[85%]">
          <div className={`${msg.role === 'user' ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700'} p-3 rounded-lg break-words overflow-hidden`}>
        ...
        {msg.role === 'user' && <div className="w-8 h-8 rounded-full flex-shrink-0">
            {isAuthenticated && user?.avatar_url ? <img src={user.avatar_url} alt="User avatar" className="w-full h-full rounded-full" /> : <div className="w-full h-full rounded-full bg-gray-500 flex items-center justify-center">
  ...
  return <>
      {isExpanded && showSidebar && <div className="fixed bottom-4 right-[470px] w-72 h-[600px] bg-white dark:bg-gray-800 rounded-lg shadow-lg flex flex-col">
          <div className="p-4 border-b dark:border-gray-700 flex items-center justify-between">
            <h3 className="text-lg font-semibold">Chat History</h3>
            <button onClick={() => setShowSidebar(false)} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full">
              <X className="w-5 h-5" />
            </button>
          </div>
          <div className="flex-1 overflow-y-auto p-4">
            {chatHistory?.length ? <div className="space-y-2">
                {chatHistory.map(chat => <div key={chat.id} className="flex items-center justify-between p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-colors group">
                    <button onClick={() => loadChat(chat)} className="flex-1 text-left">
                      <p className="text-sm font-medium truncate">{chat.title}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">
                        {new Date(chat.created_at).toLocaleDateString()}
                      </p>
                    </button>
                    {user?.id && <button onClick={e => {
              e.stopPropagation();
              deleteChat(chat.id);
            }} className="p-2 text-gray-500 hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity" aria-label="Delete chat">
                        <TrashIcon className="w-4 h-4" />
                      </button>}
                  </div>)}
              </div> : <p className="text-center text-gray-500 dark:text-gray-400">
                No chat history found
              </p>}
          </div>
        </div>}

      <div className={`fixed bottom-4 right-4 shadow-lg flex flex-col transition-all duration-300 ${isExpanded ? 'w-[450px] h-[600px] rounded-lg bg-white dark:bg-gray-800' : 'w-16 h-16 rounded-full border-2 border-blue-500 bg-gray-50 dark:bg-gray-700'}`}>
        <div onClick={toggleExpanded} className={`flex items-center justify-between cursor-pointer transition-colors ${isExpanded ? 'p-4 border-b dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50' : 'w-full h-full rounded-full hover:bg-gray-100 dark:hover:bg-gray-600'}`}>
          {isExpanded ? <>
