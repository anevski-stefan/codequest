...
const Profile = () => {
  ...
  return <div className="flex flex-col md:flex-row flex-1 bg-gray-50 dark:bg-gray-800 mt-8">
      {}
      <div className="md:hidden px-4 mb-4">
        <select onChange={e => navigate(e.target.value)} value={location.pathname} className="w-full p-2 border rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white">
      ...
      <div className="hidden md:block w-64 bg-white dark:bg-gray-800 shadow-sm border-r border-gray-200 dark:border-gray-700">
        <div className="p-6">
          <div className="flex flex-col items-center">
            <img src={user?.avatar_url} alt="Profile" className="w-24 h-24 rounded-full border-2 border-gray-200 dark:border-gray-600" />
          ...
          <nav className="mt-8">
            <ul className="space-y-2">
              {navigationItems.map(item => <li key={item.path}>
                  <NavLink to={item.path} end={item.path === '/profile'} className={({
                isActive
              }) => `flex items-center px-4 py-2 rounded-lg transition-colors ${isActive ? 'bg-blue-50 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400' : 'text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
