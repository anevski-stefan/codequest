...
import { Key, X, Monitor, Sun, Moon, Check, Loader2, Sparkles, Palette, Bot } from 'lucide-react';
        ...
        <div className="flex items-center gap-1 border-b border-white/[0.05]">
          {SECTIONS.map(({ id, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon className={`w-3.5 h-3.5 ${active ? 'text-blue-400' : 'text-gray-600'}`} />
                {label}
              </button>
            );
          })}
          ...
          <div className="divide-y divide-white/[0.05]">
            ...
            <div className="flex items-start justify-between gap-8 px-6 py-6">
              <div className="w-64 shrink-0">
                ...
                <p className="text-xs text-gray-600 mt-1 leading-relaxed">Choose how Code Quest looks on this device. Your preference is stored locally.</p>
              </div>
                ...
                {THEME_OPTIONS.map(({ value, label, icon: Icon }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex-1 flex flex-col items-center gap-3 py-6 rounded-xl border text-sm font-medium transition-all cursor-pointer ${
                        active
                          ? 'border-blue-500/50 bg-blue-500/[0.08] text-white'
                          : 'border-white/[0.07] bg-[#0D1525] text-gray-500 hover:border-white/[0.13] hover:text-gray-300'
                      }`}
                    >
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center ${active ? 'bg-blue-500/20' : 'bg-white/[0.04]'}`}>
                        <Icon className={`w-4.5 h-4.5 ${active ? 'text-blue-400' : 'text-gray-600'}`} />
                      </div>
            ...
            <div className="flex items-center justify-between px-6 py-4">
              <p className="text-xs text-gray-700">Changes are applied immediately after saving</p>
