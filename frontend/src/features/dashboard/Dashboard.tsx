                      ...
                      <div className="flex flex-col md:flex-row md:items-start justify-between">
                        ...
                        <div className="flex flex-wrap gap-1.5 mt-2 md:mt-0 md:ml-4">
                          {issue.labels.length > 10 ? <>
                              {issue.labels.slice(0, 10).map(label => <span key={label.name} className="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full whitespace-nowrap" style={getLabelColors(label.color)}>
                                  {label.name}
                                </span>)}
                              <button onClick={e => {
                      e.preventDefault();
