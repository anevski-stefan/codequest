...
const Dashboard = () => {
  ...
  return <div className="flex min-h-screen w-full">
      <div className="w-full">
        ...
        <div className="p-3 md:p-4">
          {showLoadingSpinner ? <div className="min-h-[200px]">
              <LoadingSpinner />
              ...
              {allIssues?.length > 0 && <div className="space-y-3 md:space-y-4">
                  {allIssues.map(issue => <div key={`${issue.repository?.fullName}-${issue.number}`} className="relative py-3 md:py-4 group">
                          ...
                          <div className="flex flex-col md:flex-row md:items-start justify-between">
                            ...
                            <div className="flex flex-wrap gap-1.5 mt-2 md:mt-0 md:ml-4">
                              {issue.labels.length > 10 ? <>
                                  {issue.labels.slice(0, 10).map(label => <span key={label.name} className="inline-flex items-center px-2 py-0.5 text-xs font-medium rounded-full whitespace-nowrap max-w-[120px]" style={getLabelColors(label.color)} title={label.name}>
