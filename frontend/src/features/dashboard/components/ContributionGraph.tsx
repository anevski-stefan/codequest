...
const ContributionGraph = ({
  ...
  return <div className="flex items-end h-32 gap-1">
      {data.map((value, index) => {
      const height = value ? value / maxValue * 100 : 0;
      return <div key={index} className="flex-1 bg-blue-100 rounded-t hover:bg-blue-200 transition-colors relative group" style={{
        height: `${height}%`
      }}>
