...
const ActivityFeed = () => {
  ...
  if (isLoading) {
    return <div className="animate-pulse space-y-4">
        {[...Array(5)].map((_, i) => <div key={i} className="h-16 bg-gray-100 dark:bg-gray-800 rounded"></div>)}
      </div>;
  }
  if (error) {
  ...
  return <div className="space-y-4 p-4 rounded-lg">
      {activities.map((activity: Activity) => <div key={activity.id} className="flex items-start space-x-3 p-4 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
          ...
          <div className="flex-1">
            ...
            {activity.payload.commits && <div className="mt-1 text-sm text-gray-500 dark:text-gray-400">
