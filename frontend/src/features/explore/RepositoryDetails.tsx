...
const RepositoryDetails = () => {
  ...
  } = useQuery<Repository>(['repository', owner, repo], () => getRepositoryDetails(owner!, repo!));
  const {
    data: topContributors
  } = useQuery<TopContributor[]>(['top-contributors', owner, repo], () => getTopContributors(owner!, repo!));
  const {
    data: lotteryContributors
  } = useQuery<LotteryContributor[]>(['lottery-contributors', owner, repo], () => getLotteryContributors(owner!, repo!));
  const {
    data: contributorConfidence
  } = useQuery<ContributorConfidence>(['contributor-confidence', owner, repo], () => getContributorConfidence(owner!, repo!));
  ...
  const {
    ...
    hasMore: boolean;
    totalCount: number;
  }>(['pull-requests', owner, repo, prState, page], () => getRepositoryPullRequests(owner!, repo!, prState, page), {
    enabled: !!owner && !!repo,
    ...
    onSuccess: newData => {
      if (page === 1) {
        ...
        setAllPullRequests(prev => {
          ...
          const newUniquePRs = newData.pullRequests.filter(pr => !existingIds.has(pr.id));
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div className="relative w-24 h-24">
                      <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
                        {}
                        <path d="M 50,50 m 0,-40 a 40,40 0 1 1 0,80 a 40,40 0 1 1 0,-80" fill="none" stroke="#E5E7EB" strokeWidth="8" />
                        {}
                        <path d="M 50,50 m 0,-40 a 40,40 0 1 1 0,80 a 40,40 0 1 1 0,-80" fill="none" stroke="#3B82F6" strokeWidth="8" strokeLinecap="round" strokeDasharray={`${(contributorConfidence?.percentage || 0) * 2.51} 251.2`} />
                      </svg>
                      <div className="absolute inset-0 flex items-center justify-center">
              ...
              scale: 1.02
                ...
                <div className="mb-2">
                  <svg className="w-full h-10" viewBox="0 0 400 40" preserveAspectRatio="xMidYMid meet">
                    {}
                    <line x1="40" y1="20" x2="360" y2="20" stroke="#E5E7EB" strokeWidth="1.5" />
                    {}
                    {[0, 2.5, 5, 7.5, 10].map((score, index) => <g key={score}>
                        <line x1={40 + index * 80} y1="15" x2={40 + index * 80} y2="25" stroke="#E5E7EB" strokeWidth="1.5" />
                        <text x={40 + index * 80} y="35" textAnchor="middle" className="text-[11px] fill-gray-500">
                          {score}
                        </text>
                      </g>)}
                    {}
                    <circle cx={40 + 5.4 / 10 * 320} cy="20" r="4" fill="#059669" stroke="#FFFFFF" strokeWidth="2" />
                  </svg>
                </div>
              ...
              scale: 1.02
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    The top {lotteryContributors?.length || 0} contributors of this repository have made {lotteryContributors?.reduce((sum, c) => sum + c.percentage, 0) || 0}% of all pull requests in the past 30 days.
                  </p>
          ...
          <div className="mt-8">
            ...
            <div className="space-y-4">
              {prsLoading && page === 1 ? <LoadingSpinner /> : allPullRequests?.length === 0 ? <div className="text-center py-8 text-gray-500 dark:text-gray-400">
                ...
                y: 0
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center space-x-2">
                          {pr.draft && <span className="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300">
                              Draft
                            </span>}
                          <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${pr.state === 'open' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400'}`}>
                      ...
                      {pr.labels.length > 0 && <div className="mt-4 flex flex-wrap gap-2">
                          {pr.labels.map(label => <span key={label.name} className="px-2 py-1 rounded-full text-xs font-medium" style={{
                    ...
                    color: `#${label.color}`
                      ...
                      {pr.requested_reviewers.length > 0 && <div className="mt-4">
                          <div className="text-sm text-gray-500 mb-2">Reviewers</div>
                          <div className="flex -space-x-2">
                            {pr.requested_reviewers.map(reviewer => <img key={reviewer.login} src={reviewer.avatar_url} alt={reviewer.login} title={reviewer.login} className="w-8 h-8 rounded-full border-2 border-white dark:border-gray-800" />)}
                          </div>
                        </div>}
        ...
        <div className="space-y-6">
          ...
          delay: 0.3
            ...
            <div className="space-y-4">
              ...
              {topContributors && topContributors.length > 5 && <button className="w-full py-2 text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors">
