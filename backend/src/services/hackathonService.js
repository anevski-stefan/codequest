...
class HackathonService {
  ...
  async crawlEventbrite() {
    try {
      ...
      const response = await axios.get('https://www.eventbrite.com/d/online/hackathon/?page=1', this.axiosConfig);
      const $ = cheerio.load(response.data);
      const hackathons = [];
  ...
  parseMlhDate(dateStr) {
    try {
      ...
      if (parts.length === 2) {
        ...
        return [this.formatDate(startDate.toLocaleDateString()), this.formatDate(endDate.toLocaleDateString())];
      }
